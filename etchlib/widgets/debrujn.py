...
class Debrujn(QGraphicsObject):
    ...
    def __init__(self,w=1000,scale=1.0,parent=None):
        ...
        self.path.cubicTo(99, 0,  50, 50,  99, 99)
        self.path.cubicTo(0, 99,  50, 50,  0, 0)
    ...
    def hoverMoveEvent(self,event):
        ...
        super(Debrujn,self).hoverMoveEvent(event)
        self.update()

    def mouseMoveEvent(self,event):
        self.currentMovePosition = event.pos()
        super(Debrujn,self).mouseMoveEvent(event)
        self.update()
        ...
        if event.button() == Qt.RightButton:
            ...
            if hasattr(self,'rightClickF'):
                for f in self.rightClickF:
                    f(event)
        super(Debrujn,self).mousePressEvent(event)
        self.update()
